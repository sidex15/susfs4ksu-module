...
if echo "$susfs_features" | grep -q "CONFIG_KSU_SUSFS_OPEN_REDIRECT"; then
	grep -v "#" "$PERSISTENT_DIR/sus_open_redirect.txt" | while read -r line; do
		...
		redirected_ino=$({ stat -c %i "$redirected_path" 2>/dev/null || echo "0"; })
		redirected_dev=$({ stat -c %d "$redirected_path" 2>/dev/null || echo "0"; })
		...
		${SUSFS_BIN} add_sus_kstat_statically "$original_path" "$redirected_ino" "$redirected_dev" 'default' 'default' 'default' 'default' 'default' 'default' 'default' 'default' 'default' 'default' && echo "[add_sus_kstat_statically]: susfs4ksu/boot-completed $original_path" >> $logfile1
	done
fi
